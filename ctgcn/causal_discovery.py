...
class DecomposedCausalDiscovery(CausalDiscovery):
    def __init__(self, data_path: str, tau_max: float, var_names: Optional[List[str]] = None, pc_alpha: Optional[float] = 0.01,
        ...
        print(f"    decomp_period: {decomp_period}")
        if ci_test.lower() == 'gpdc':
            self.ci_test = GPDC(significance='analytic')
        if ci_test.lower() == 'gpdctorch':
            self.ci_test = GPDCtorch(significance='analytic')
    ...
    def decompose(self, data):
        ...
        try:
            ...
            if self.decomp_clusters and self.one_cluster:
                print(f'Compute global cluster')
                ...
                print(f'Global cluster computed')

            #for i, (start, end) in enumerate(tqdm(zip(periods, periods[1:]),total=len(periods), ascii=True, file=sys.stdout)):
            for i, (start, end) in enumerate(zip(periods, periods[1:])):
                print(f'\nTemporal split {i} of {len(periods) - 1} with shape {data[start:end, :].shape}')
                ...
                if not self.decomp_clusters:
                    ...
                    if not self.one_cluster:
                        ...
                        if self.verbose > 0:
                            print(f'\n  Clustering inertia: {km.inertia_}')
                    ...
                    for cluster, idxs in cluster_map.items():
                        try:
                            ...
                            print(f'Error in computing cluster:', e)
                ...
                self.stats["runtime_total"] = (dtnow - self.starttime).total_seconds()
                self.stats["endtime"] = dtnow.isoformat()

                if self.result_path:
                    with open(os.path.join(self.result_path, f"stats.json"), "wt") as fo:
    ...
    def save(self, result_path: str=None):
        ...
        if self.result_path:
            np.save(os.path.join(self.result_path, f"results.np"), self)
            with open(os.path.join(self.result_path, f"stats.json"), "wt") as fo:
                json.dump(self.stats, fo)
